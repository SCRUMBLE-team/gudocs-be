...
- [ ] 프로젝트 한 줄 설명
    - 구독 서비스 정보를 통합 관리하고, 월별 구독 지출과 결제일을 확인할 수 있는 웹 기반 대시보드 시스템
- [ ] 로컬 실행 명령어
- [ ] 빌드 명령어
- [ ] 테스트 실행 명령어
- [ ] 프론트엔드 / 백엔드 실행 방법 구분
- [ ] 주요 도메인 목록
    - `auth` / 회원가입, 로그인, 로그아웃, 비밀번호 찾기
    - `subscription` / 구독 서비스 등록, 목록 조회, 상세 조회, 수정, 삭제, 일시 정지
    - `expense` / 월별 총 구독 지출, 카테고리별 지출 비율, 월별 지출 추이
    - `notification` / 결제 예정 알림
    - `dashboard` / 메인 대시보드 조회
    - `member` / 회원 정보 수정, 비밀번호 변경, 회원 탈퇴
- [ ] API 계층 구조
    - Controller → Service → Repository 구조를 사용하는 경우 명시
- [ ] 화면 흐름
    - 홈 → 회원가입/로그인 → 대시보드 → 구독 관리 / 지출 분석 / 마이페이지
- [ ] 구독 서비스 상태 관리 규칙
    - 활성, 일시 정지, 삭제 상태 구분
- [ ] 결제일 기반 알림 규칙
    - 7일 전, 3일 전, 1일 전 알림 등
...
- [ ] application.yml, .env 내용 직접 기재
- [ ] DB 비밀번호, API Key, JWT Secret 등 민감 정보
- [ ] 코드 스니펫 직접 삽입
- [ ] 추측성 설명 또는 미확인 동작 방식
...
├── CLAUDE.md              # 팀 공유 Claude 지침 → 루트에, git 커밋
├── AGENTS.md              # 모든 AI 에이전트 공통 가이드 → 루트에, git 커밋
├── AI_DOCS_GUIDE.md       # 이 파일
├── .claude/               # Claude Code 도구 설정
│   ├── settings.json
│   ├── agents/
│   └── skills/
└── CLAUDE.local.md        # 개인 전용 메모 → .gitignore에 추가, 커밋 X
