...
> **정책 정정(2026-08-11):** 이 문서의 `actualAmount`는 카드·은행에서 확인한 실제 승인 금액이
> 아니라 구독 등록정보의 금액·주기로 계산한 청구 예정액이었다. 현재 API는 호환을 위해 JSON 이름
> `actualAmount`를 유지하지만, 서버 내부 의미는 `recordedBillingAmount`(기록 청구액)이다.
> `BillingRecord` 역시 실제 결제 증빙으로 사용하지 않는다.
