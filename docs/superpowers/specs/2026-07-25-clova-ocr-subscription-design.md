...
`billingCycle`을 제외한 모든 필드는 nullable이다. 파싱에 실패한 필드는 `null`로
내려가고, FE는 해당 필드를 사용자가 직접 입력하게 한다. `billingCycle`만 예외로,
인식 실패 시에도 `null` 대신 `MONTHLY`를 기본값으로 채운다(구독 등록 폼에서 결제
주기를 비워두지 않도록 하기 위함).
