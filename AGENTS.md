...
- **가격 변경 예고(`Plan.change`)도 이 파일이 단일 소스**다. 공식 인상·인하 발표가 나오면 해당 요금제에 `changingTo(신가격, 적용일, 발표일, 공식출처URL)`를 붙인다. **이 파일에 적혀 있다는 것 자체가 "사람이 공식 출처로 검증했다"는 뜻**이고, 그래서 `DETECTED/VERIFIED` 같은 검증 상태나 관리자 화면을 두지 않는다 — 검증 절차는 이 파일을 고치는 PR 리뷰다. 채우는 규칙:
  - **구가격(`oldPrice`)을 명시적으로 적는다.** `price`는 "지금의 공식가"라 적용일에 새 금액으로 올라가고, 그 뒤에는 구가격을 알 방법이 없어진다. 알림 대상은 **아직 구가격을 쓰고 있는 구독**이므로 이 값이 사라지면 안 된다.
  - 출처는 **공식 공지·요금제 페이지·고객센터 문서**만. 블로그·기사만 있는 소문은 적지 않는다(`ServiceCatalogTest`가 `https://` 여부까지 검사한다).
  - **적용일에 하는 일은 `price` 숫자를 새 금액으로 바꾸는 것뿐이고, 선언은 지우지 않는다.** 사용자마다 결제일이 달라 실제 청구는 적용일보다 최대 한 달(연간 결제면 최대 1년) 늦게 일어나고, 그때 "금액 확인하셨나요"를 물어야 하기 때문이다. 적용 전에는 `price == oldPrice`, 적용 후에는 `price == newPrice`가 되며 둘 다 아니면 `ServiceCatalogTest`가 잡는다. 모든 구독이 한 바퀴 돈 뒤(연간 결제까지 고려하면 13개월) 선언을 지운다.
...
- **④ 가격 변경 알림 (`PRICE_CHANGE`)**: 스케줄러(별도 cron `FCM_PRICE_CHANGE_CRON`) → `PriceChangeDispatchService.dispatchDeclaredPriceChanges(today)` → `ServiceCatalog.declaredPriceChanges()`(카탈로그에 선언된 공식 인상·인하 예고)를 훑어 **아직 구가격을 쓰는 활성 구독**에 **구독별 1건** 발송. 한 변경 건당 구독마다 **두 단계**를 각 1회씩 보낸다(단계 = dedup 키의 `remind_offset`).
  - **① 발표(offset 0, 적용일까지)** — "넷플릭스 요금이 변경될 예정이에요" / "프리미엄 요금제가 9월 1일부터 19,000원으로 인상될 예정이에요. 공식 안내를 확인해보세요." 제목은 인상·인하 공통이고 **본문이 갈린다**. 클릭하면 **서비스의 공식 안내 페이지(외부 URL)**로 나간다 — 가격 변경은 원문 확인이 가장 확실하다
  - **② 금액 확인(offset 1, 적용일 이후 그 구독의 결제일이 지난 뒤)** — "이번 넷플릭스 결제 금액, 확인하셨나요?" / "최근 공식 요금이 19,000원으로 변경됐어요. 실제 결제 금액이 달라졌다면 업데이트해주세요." 클릭하면 **구독 수정 화면**(`/subscriptions/{id}/edit`)으로 보낸다. 결제일 판정은 `NextBillingDateCalculator`로 적용일 이후 첫 결제일을 구해 오늘과 비교한다(새 컬럼 없음)
  - **적용일에 일괄로 묻지 않는 이유**: 사용자마다 결제일이 달라 실제 청구 시점이 최대 한 달(연간 결제면 최대 1년) 늦다. 적용일에 물으면 아직 옛 금액을 내고 있는 사람에게 확인을 요구하게 된다
  ...
  - **반영은 사용자 몫이고 서버는 금액을 자동으로 바꾸지 않는다.** 기존가 유지·프로모션·제휴결합·인앱결제 사용자는 적용일이 지나도 계속 옛 금액을 낸다. 서버가 대신 고치면 본인이 아무것도 하지 않았는데 지출 분석이 조용히 틀리고 알릴 방법도 없다. 실제 금액을 아는 사람은 사용자뿐이라 수정 화면으로 보내 직접 확인하게 한다. **수정하고 나면 구가격과 일치하지 않아 대상에서 저절로 빠진다** — "확인했음" 상태를 저장하는 컬럼이 필요 없다(무시하고 닫는 경우의 재노출 억제는 FE 몫)
