...
- **FCM payload** — notification: `title`/`body`는 알림 종류·묶음 수에 따라 구성(위 참고) / data(모두 문자열): `type`(BILLING_REMINDER|SUBSCRIPTION_REVIEW|CANCEL_REMINDER), `link`, (단일 구독을 가리키는 알림만) `subscriptionId`. FE는 `data.link`를 그대로 열면 된다(절대 URL). 이동 경로: CANCEL_REMINDER→`/subscriptions/{id}` 구독 상세, BILLING_REMINDER→**단건이면 `/subscriptions/{id}` 상세, 묶음이면 `/notifications` 알림함**(클릭 액션은 하나뿐이라 여러 건 중 하나를 고를 수 없음), SUBSCRIPTION_REVIEW→`/subscriptions` 구독 점검. **`subscriptionId`는 optional** — 묶음/유저 단위 알림에는 없다
