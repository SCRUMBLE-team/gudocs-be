 ...
 * 구가격을 알 방법이 없어지는데, 서비스 재접속 배너는 그 뒤에도 "17,000원 → 19,000원"을
 * 보여줘야 하기 때문이다. 배너 노출 시점은 {@link SubscriptionResponse#priceReviewRequired()}가 결정한다.
 *
 * <p><b>서버는 사용자의 결제 금액을 자동으로 바꾸지 않는다.</b> 기존가 유지·프로모션·제휴결합·인앱결제로
 * 사람마다 실제 청구액이 달라서다. 프론트는 {@code priceReviewRequired=true}일 때만 확인 배너를
 * 보여주고, 사용자가 반영을 고르면 기존 구독 수정 API로 금액을 바꾼다. 이 확인을 위한 별도 푸시는
 * 발송하지 않는다.
 *
 * <p><b>반영하고 나면 이 필드는 저절로 사라진다</b> — 구독의 금액이 더 이상 변경 대상 요금제의
 * 금액과 일치하지 않기 때문이다. 따로 "확인했음" 상태를 저장하지 않아도 배너 대상에서 빠진다.
 */
